...
func TestImageGCTreeStore(t *testing.T) {
	...
	// We expect 2 treeStoreIDs for stage1 and app
	if len(treeStoreIDs) != 2 {
		t.Fatalf("expected 2 entries in the treestore but found %d entries", len(treeStoreIDs))
	}
	...
	// We expect 2 treeStoreIDs again as no pod gc has been executed
	if len(treeStoreIDs) != 2 {
		t.Fatalf("expected 2 entries in the treestore but found %d entries", len(treeStoreIDs))
	}
